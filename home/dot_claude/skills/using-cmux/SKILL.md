...
git の差分をブラウザ split で見られる。unstaged/staged/branch/last-turn の4種。

```bash
cmux diff --unstaged                              # 作業ツリーの未ステージ差分
cmux diff --staged                                # ステージ済み差分
cmux diff --branch --base main                    # ブランチ差分（base 省略時は origin/HEAD か main）
cmux diff --last-turn                             # このサーフェスの直前エージェントターン以降の差分
cmux diff --cwd <path> --unstaged                 # 別リポジトリ/worktree を対象に
```

`--repo` は `--cwd` の別名。詳細は `cmux diff --help` を参照。

### diff ビューアのコメントを取得する

`cmux diff` で開いたビューアにはインラインコメント機能がある。中身は `browser snapshot` で目視するより、`browser eval` で DOM を直接叩くほうが速く確実。

```bash
cmux browser --surface <diffビューアのsurface> eval "
JSON.stringify(Array.from(document.querySelectorAll('.comment-entry')).map(el => ({
  location: el.querySelector('.comment-entry-location')?.textContent.trim(),
  message: el.querySelector('.comment-entry-message')?.textContent.trim()
})))
"
```

`.comment-entry`（サイドバー一覧、1コメント1要素）に `.comment-entry-location`（`ファイル名:行番号`）と `.comment-entry-message`（本文）が入っている。インライン表示側は `.comment-card` / `.comment-card-location` / `.comment-card-actions` で同内容。
