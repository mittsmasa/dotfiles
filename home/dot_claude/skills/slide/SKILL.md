...
- **同じ図をページ間で再掲するときは、同じ位置・同じサイズで出す**。図の再登場は「あの話がこう変わる」の対比装置なので、位置がズレると対比が壊れる。積み増し系(吹き出しが増える等)は、要素が無いページでも同じグリッドを敷いて位置を固定する(Key Message が無いページには同じ高さの透明スペーサーを置く)
...
**ハイライトボックス（ダーク）**: bg: #1A1A1A, color: #FFF, border-radius: 8px。図の要素(プロセスのノード、循環図の箱等)専用。紙面の文の強調には使わない(図と見た目が被るため)。
