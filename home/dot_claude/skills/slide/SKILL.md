...
- **SVG図は `flex:1` コンテナに収まる高さを事前に計算する**。`flex:1` の子要素は縮まず、SVGの `width`/`height` 属性がそのまま描画サイズになる。見出し・本文・Key Message の高さを差し引いた残り領域より図が大きいと、Key Message が紙面下にはみ出す。図が大きい場合は `viewBox` は変えずに `width`/`height` 属性だけ縮小して収める
- **同じ概念を表す図の要素は、ページを跨いでも同じ形にする**。「AIエージェント」のような繰り返し登場する概念を、あるページでは角丸バー、別のページでは丸ノードのように描き分けない。形が変わると同一の存在だと認識されず、対応関係が伝わらない
- **矢印は指す先の具体的な要素まで正確に届かせる**。箱の中の特定のラベル(知識チップ等)を指したいなら、箱全体の縁ではなくラベルの位置まで伸ばす。ただし他の要素(役割名など)を突っ切らないよう、途中に浮いた矢印にならないか確認する。座標は勘で置かず、[html2pdf.md](references/html2pdf.md) の実測手順で終点の実座標を取ってから当てはめる
...
**区画 + チップ**: 区画は border: 2px dashed #AAA の白背景(範囲・領域を表す)。中の概念ラベル(チップ)はダーク(黒)で図のノード扱い。区画・吹き出し・Key Message は3つとも別の見た目にする(グレー背景を共有すると役割が判別できなくなる)。区画同士の独立は間隔(gap)だけで示す。あいだに黒バー等の「壁」要素を足さない(破線の枠がすでに閉じていることを示しており、壁は同じ意味の重複表現になる)。

**ツールアイコン(知識のありか等)**: Confluence / Figma / GitHub のような実サービスを図に添えるとき、各社ロゴ画像は使わない(取得・ライセンスの手間があり、モノトーン紙面で色が浮くため)。頭文字1〜2字の小さな角丸タイル + ツール名テキスト、のようなモノトーンのテキストベース簡易アイコンで描く。
