...
| plan.md / verify-results.md 状態 | PR / dirty | 列 |
|---|---|---|
| 何も無し or `Approval Status: approved` | - | In Progress |
...
| PR が紐付いている | open | PR Open |
| PR が紐付いている | merged | Done |

dashboard 側の判定実装は `server.ts` の `derivePhase`。挙動を変えたいときは表と関数の両方を更新する。
